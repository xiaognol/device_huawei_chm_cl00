...
#define CHM_CL_PLATFORM_INFO_XML_PATH "/system/etc/sound_param/chm_cl/audio_platform_info.xml"
#define ALE_CL_PLATFORM_INFO_XML_PATH "/system/etc/sound_param/Alice/audio_platform_info.xml"
#define CHM_CL_ACDB_PATH "/system/etc/sound_param/chm_cl"
#define ALE_CL_ACDB_PATH "/system/etc/sound_param/Alice"
#define CHM_Cl_MIXER_XML_PATH_SKUK "/system/etc/sound_param/chm_cl/mixer_paths.xml"
#define ALE_Cl_MIXER_XML_PATH_SKUK "/system/etc/sound_param/Alice/mixer_paths.xml"
#define DEFAULT_ACDB_PATH "/system/etc"
...
bool huawei_path();
bool huawei_platform_info_path();
bool huawei_mixer_path();
    ...
    if (!strncmp(snd_card_name, "msm8x16-snd-card-mtp",
        ...
        msm_be_id_array_len  =
            sizeof(msm_device_to_be_id_internal_codec) / sizeof(msm_device_to_be_id_internal_codec[0]);
                 ...
                 sizeof("msm8939-snd-card-skuk"))) & (huawei_mixer_path() == false)) {
        strlcpy(mixer_xml_path, ALE_Cl_MIXER_XML_PATH_SKUK,
                sizeof(ALE_Cl_MIXER_XML_PATH_SKUK));
        msm_device_to_be_id = msm_device_to_be_id_internal_codec;
        msm_be_id_array_len  =
            sizeof(msm_device_to_be_id_internal_codec) / sizeof(msm_device_to_be_id_internal_codec[0]);

    } else if ((!strncmp(snd_card_name, "msm8939-snd-card-skuk",
                 sizeof("msm8939-snd-card-skuk"))) & (huawei_mixer_path() == true)) {
        strlcpy(mixer_xml_path, CHM_Cl_MIXER_XML_PATH_SKUK,
                sizeof(CHM_Cl_MIXER_XML_PATH_SKUK));
        msm_device_to_be_id = msm_device_to_be_id_internal_codec;
        msm_be_id_array_len  =
            sizeof(msm_device_to_be_id_internal_codec) / sizeof(msm_device_to_be_id_internal_codec[0]);
...
void *platform_init(struct audio_device *adev)
{
    ...
    if (my_data->acdb_handle == NULL) {
        ...
        if (!my_data->acdb_set_param_path)
			...
			if (huawei_path() == false) {
            		my_data->acdb_set_param_path(ALE_CL_ACDB_PATH);
			} else {
			my_data->acdb_set_param_path(CHM_CL_ACDB_PATH);
			}
#endif
...
acdb_init_fail:
	...
	if (huawei_platform_info_path() == false) {
    	platform_info_init(ALE_CL_PLATFORM_INFO_XML_PATH);
	} else {
	platform_info_init(CHM_CL_PLATFORM_INFO_XML_PATH);
	}
    /* init usb */
...
bool huawei_platform_info_path()
{
    ...
    if (strstr(model, "ALE-CL00") != NULL) {
        ALOGW("HUAWEI PROFILE PLATFORM INFO = %s", ALE_CL_PLATFORM_INFO_XML_PATH);
        return false;
    }
	else if (strstr(model, "CHM-CL00") != NULL) {
        ALOGW("HUAWEI PROFILE PLATFORM INFO = %s", CHM_CL_PLATFORM_INFO_XML_PATH);
        return true;
    }
...
bool huawei_mixer_path()
{
    ...
    if (strstr(model, "ALE-CL00") != NULL) {
        ALOGW("HUAWEI PROFILE PLATFORM INFO = %s", ALE_Cl_MIXER_XML_PATH_SKUK);
        return false;
    }
	else if (strstr(model, "CHM-CL00") != NULL) {
        ALOGW("HUAWEI PROFILE PLATFORM INFO = %s", CHM_Cl_MIXER_XML_PATH_SKUK);
        return true;
    }
